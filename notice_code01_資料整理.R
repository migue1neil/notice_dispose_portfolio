...
#####上櫃注意股#####
#載入上櫃注意股、警示股
OTC_notice = fread("./orign_data/封存/上櫃注意股_20111001_20221019.csv" , sep = ",", header = T , encoding = "UTF-8" )
...
write.csv(all.stat.sheet , "./tidy_up_data/注意股結果/全市場注意股出現次數表格.csv" , row.names = F )
...
####載入上櫃處置股####
OTC_dispose = fread("./orign_data/封存/上櫃處置股_20111001_20221019.csv")
...
#上面是白癡分組寫法
stock_price = stock_price %>% mutate( 條款分類 = ifelse( grepl("成交量組_其他",條款分類), "其他" , 條款分類 ),
                                      條款分類 = ifelse( grepl("漲跌組_成交量組",條款分類), "成交量漲跌組" , 條款分類 ),
                                      條款分類 = ifelse( grepl("漲跌組_成交量組_其他",條款分類), "其他" , 條款分類 ),
                                      條款分類 = ifelse( grepl("漲跌組_成交量漲跌組",條款分類), "成交量漲跌組" , 條款分類 ),
                                      條款分類 = ifelse( grepl("漲跌組_成交量漲跌組_其他",條款分類), "其他" , 條款分類 )
                                      ) %>% 
...
for (ndays in 0:10){
  if (ndays == 0){ stock_price = stock_price %>% group_by(證券代碼) %>% 
    mutate( 第0日報酬 = (調整收盤價 - lag(調整收盤價,n=1)) / lag(調整收盤價,n=1) ) } %>% ungroup #持有0日是偷看
  if(ndays != 0){
    stock_price = stock_price %>% group_by(證券代碼) %>% 
      mutate( holding_ndays = ( lead(調整收盤價, n = ndays ) - lead(調整收盤價, n = ndays-1   ) )/lead(調整收盤價, n = ndays-1  )) %>% ungroup
    colnames(stock_price)[NCOL(stock_price)]  = paste0("第",ndays,"日報酬")
  }
}

# 筆記 : 要用shift ，(今天收盤價-昨天收盤價)/昨天收盤價
#                     lead( 調整收盤價 , n = ndays ) - lead( 調整收盤價 , n = ndays-1 )  #shift(-1) = lead(1)
...
for (ndays in 1:10){
  if (ndays == 0){ stock_price = stock_price %>% group_by(證券代碼) %>% 
    mutate( 持有0日報酬 = (調整收盤價 - lag(調整收盤價,n=1)) / lag(調整收盤價,n=1) ) } %>% ungroup #持有0日是偷看
  if(ndays != 0){
    stock_price = stock_price %>% group_by(證券代碼) %>% 
      mutate( holding_ndays = ( lead(調整收盤價, n = ndays ) - lead(調整收盤價, n = ndays-1   ) )/lead(調整收盤價, n = ndays-1  )) %>% ungroup
...
tmp_stock_price = stock_price %>% filter( grepl( "第六款"  , conditions ) )

tmp_stock_price = stock_price %>% filter(公司名稱 == "大飲")

#測試
all_dispose$證券代碼test = shift(all_dispose$證券代碼,1)
