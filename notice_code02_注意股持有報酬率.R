...
each_notice_daily_func = function(stock_price , market){   #market = "TSE"
  ...
  for(law in dic){
    ...
    for(vol in vol.list){ #篩選出股票順便更具第二款分類
      ...
      sheet = data.table()
      for(i in 16:37){ #算風險指標 第0日報酬~第10日報酬
        a.vector = tmp[[i]] 
...
# ccd = TSE_notice_stat_sheet %>% sub( "NA" , "不分類" , x = vol)
# print(ccd)
...
# 
# #分漲跌 #其實近六日漲跌幅只適用 1,3,4,5,7,11 款，其他的要想辦法刪掉
# early_TSE_vol_upside = early_hold.law.return.func(stock_price , market = "TSE" ,volatility = "+")
# early_TSE_vol_downside = early_hold.law.return.func(stock_price , market = "TSE" ,volatility = "-") 
# #分漲跌 #
# early_OTC_vol_upside = early_hold.law.return.func(stock_price , market = "OTC" ,volatility = "+")
# early_OTC_vol_downside = early_hold.law.return.func(stock_price , market = "OTC" ,volatility = "-")
